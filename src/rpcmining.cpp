...
Value getnetworkhashps(const Array& params, bool fHelp)
{
    ...
    return GetNetworkHashPS(params.size() > 0 ? params[0].get_int() : 60, params.size() > 1 ? params[1].get_int() : -1);
}
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "zcashshare is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "zcashshare is downloading blocks...");
